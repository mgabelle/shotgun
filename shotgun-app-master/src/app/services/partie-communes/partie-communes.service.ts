...
export class PartieCommunesService {
  ...
  public getPartiesCommunesDisponibles(date): void {
    this.httpClient
      .get(PartieCommunesRoutes.GET_PARTIES_COMMUNES_DISPONIBLES(date))
      .subscribe((values: Array<PartieCommune>) => {
        this._PartiesCommunes.next(values);
      });
  }
  ...
  public getPartiesCommunesShotguns(): void {
    this.httpClient
      .get(PartieCommunesRoutes.GET_PARTIES_COMMUNES_SHOTGUNS())
      .subscribe((values: Array<PartieCommune>) => {
        this._PartiesCommunes.next(values);
      });
  }
